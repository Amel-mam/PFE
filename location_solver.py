...
def generer_voisin_et_move_info_ts(solution, types_manifold_data, sites_candidats, prob_moves):
    ...
    if move_type == 'reassign_well' and len(active_mfd_ids) > 1:
        ...
        candidats_dest = [mid for mid in active_mfds if mid != m_from and len(active_mfds[mid]['puits_connectes']) < types_manifold_data[active_mfds[mid]['type_id']]['capacite_puits']]
...
st.write("Cette application conçoit la structure la plus rentable pour des réseaux de collecte gaziers. Elle détermine simultanément l'emplacement le plus avantageux des manifolds et du CPF (Central Processing Facility), tout en assurant l'affectation la plus économique de chaque puits à un manifold. La solution initiale, générée par une heuristique de clustering, est ensuite affinée par des métaheuristiques (Recuit Simulé, Recherche Tabou).")
...
if uploaded_file is not None:
    ### MODIFICATION ICI ###
    scenario_choice = st.sidebar.selectbox("2. Choisissez le scénario", ["CPF à Localiser", "CPF Fixé"])
    ...
    with st.sidebar.expander("Paramètres avancés"):
        ...
        ### MODIFICATION ICI ###
        if scenario_choice == "CPF Fixé":
            st.subheader("Coordonnées CPF Fixé")
    ...
    if st.sidebar.button("🚀 Lancer l'optimisation", use_container_width=True):
        ...
        if donnees:
            ...
            else: # Cas "CPF à Localiser"
                with st.spinner("2. Recherche de la position initiale du CPF..."):
            ...
            if not sol_initiale or cout_initial == float('inf'):
                ...
                ### MODIFICATION ICI ###
                cpf_is_fixed = (scenario_choice == "CPF Fixé")
